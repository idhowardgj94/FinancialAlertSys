const
COMPANY_DIV_HEIGHT = 31;
const
STRINGLENTH = 20;
$(function() {
	$("ul.navigation > li:has(ul) > a").append(
			'<div class="arrow-bottom"></div>');
	$("ul.navigation > li ul li:has(ul) > a").append(
...
function searchCompanyValueatRisk(e, status) {
	...
	if (!lengthcheck(searchString, STRINGLENTH)) {
		...
		return false;
	} else if (isdigit(searchString) == false
			&& isChinese(searchString) == false) {
...
function searchCompany(searchString, status) {
	$.ajax({
		url : './countCompanyindex.php?search=' + searchString + '&status='
				+ status,
		type : 'GET',
		async : false,
		success : function(cindex) {
			...
			// 不同頁面則告知頁面名及跳轉頁面
				...
				case -1:
					...
					document.location.href = "listedtsec.php?search="
							+ searchString;
					break;
				case -2:
					alert(searchString + '公司在上櫃分類');
					document.location.href = "listedotcc.php?search="
							+ searchString;
					break;
				case -3:
					alert(searchString + '公司在興櫃分類');
					document.location.href = "listedces.php?search="
							+ searchString;
					break;
				case -4:
					alert(searchString + '公司在公開發行分類');
					document.location.href = "gopublicc.php?search="
							+ searchString;
					break;
				case -5:
					alert(searchString + '公司在下市櫃分類');
					document.location.href = "cdelisting.php?search="
							+ searchString;
					break;
				default:
					alert(index);
